...
int run(void) {
    ringbuffer_t *low = rb_new(low_output, sizeof(*low_output));
    if (low == NULL) {
        abort();
    }

    ringbuffer_t *high = rb_new(high_output, sizeof(*high_output));
    if (high == NULL) {
        abort();
    }

    ringbuffer_t *input = rb_new(char_in, sizeof(*char_in));
    if (input == NULL) {
        abort();
    }
